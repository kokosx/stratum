...
func TestPlacementValidation(t *testing.T){
	...
	tests := []struct{name, json string; wantFail bool}{
	...
	for _, tc:=range tests{
		t.Run(tc.name, func(t *testing.T){
			...
			if tc.wantFail && err==nil { t.Fatalf("expected fail got ok") }
			if !tc.wantFail && err!=nil { t.Fatalf("expected ok got %v", err)}
		})
	}
}
...
func TestPlacementSchemaValidation(t *testing.T){
	...
	for i,s:=range invalid{
		if _, err:= ParseSchema(s); err==nil { t.Fatalf("invalid %d should reject", i)}
	}
	if _,err:= ParseSchema(`{"schemaVersion":1,"placement":{"parents":["core/accordion"]},"props":{"type":"object","properties":{}},"settings":{"type":"object","properties":{}},"children":{"mode":"none"},"editor":{}}`); err!=nil { t.Fatalf("valid schema: %v",err)}
	if _,err:= ParseSchema(`{"schemaVersion":1,"props":{"type":"object","properties":{}},"settings":{"type":"object","properties":{}},"children":{"mode":"none"},"editor":{}}`); err!=nil { t.Fatalf("missing placement should be ok: %v", err)}
}
