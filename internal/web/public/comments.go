...
var commentFragments = template.Must(template.New("comments").Parse(`{{define "errors"}}<div id="comment-form-errors" class="form-error">{{.Message}}</div>{{end}}{{define "status"}}<div id="comment-submit-status" class="success">{{.Message}}</div>{{end}}{{define "form"}}<form id="comment-form" method="post" action="/comments" class="stratum-comment-form" data-on:submit__prevent="@post('/comments', {contentType: 'form'})"><input type="hidden" name="entry_id" value="{{.EntryID}}"><input type="hidden" name="parent_id" value=""><div id="comment-form-errors"></div><p><label>Name <input name="author_name" required maxlength="100"></label></p><p><label>Email <input name="author_email" type="email" required maxlength="254"></label></p><p><label>Website <input name="author_url" type="url" maxlength="2048"></label></p><p class="stratum-comment-honeypot"><label>Leave empty <input name="website_confirm" autocomplete="off"></label></p><p><label>Comment <textarea name="body" required maxlength="5000"></textarea></label></p><p><button type="submit">Post Comment</button></p><div id="comment-submit-status"></div></form>{{end}}{{define "list"}}<div id="comment-list">{{range .}}<article class="stratum-comment comment-depth-{{.Depth}}" id="comment-{{.ID}}"><header class="stratum-comment-header"><span class="stratum-comment-author">{{.AuthorName}}</span> <time datetime="{{.CreatedISO}}">{{.CreatedAt}}</time>{{if .ParentID}} <a href="#comment-{{.ParentID}}">in reply</a>{{end}}</header><div class="stratum-comment-body">{{.Body}}</div></article>{{else}}<p class="stratum-comments-empty">No comments yet.</p>{{end}}</div>{{end}}`))
